...
;(function (global, factory) {
  ...
  window.SplitType = (function (window, document, undefined) {
    ...
    function extend(target, object) {
      ...
      return Object.getOwnPropertyNames(target).reduce(function (
        extended,
        key
      ) {
    ...
    function createElement(name, attributes) {
      ...
      forEach(attributes, function (name) {
        ...
        switch (name) {
          // 'text' sets the splitLines content
          case 'textContent':
    ...
    function _split(element) {
      ...
      var settings = this.settings,
        ...
        isAbsolute =
          settings.position === 'absolute' || settings.absolute === true,
      ...
      tempNode.innerHTML = element.innerHTML.replace(
        /<br\s*\/?>/g,
        ' ' + BR_SYMBOL + ' '
      )
      ...
      if (isAbsolute) {
        ...
        forEach(nodes, function (node) {
          ...
          node.style.left = isLine
            ? contentBox.left + 'px'
            : (isLineChild
                ? Data(node).left - contentBox.left
                : Data(node).left) + 'px'
    ...
    SplitType.prototype.split = function split(newOptions) {
      ...
      // If new options were passed into the split() method, runAnimation settings for the instance.
